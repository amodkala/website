---
title: "Debugging Raft"
date: 2024-01-04T12:00:00-05:00
description: "Tackling a years-old deadlock bug in one of my projects"
tags:
    - "projects"
isDraft: false
---
...
When I first revisited the project, my most pressing concern was to address a race condition 
in the election process. This bug was a major blocker for any other features I would want to add, 
as without functional leader election the rest of the consensus algorithm never comes into effect.
...
<A
    ...
    text="goroutines" 
/>, with the responses being processed immediately after. The full code for the startElection 
function looked like this at the time:
...
Shoddy error handling aside, this code implements the candidate component of leader election as 
...
Another way to look at the function then, is that nearly instantly after sending our RPCs we evaluate 
the win condition, lose the election (as no votes were received instantly aside from our own) and begin 
a new election.
...
As a candidate it doesn't matter how every one of our peers respond, we only care whether we 
have received a plurality of the cluster's votes. The way I went about implementing this 
behaviour was to again use two of the primary tools used by Go programmers: 
<A
    text="channels"
...
I am inclined to keep this solution for now, as my roadmap for the project includes 
setting up a distributed testing environment where more latency is introduced and 
the effects could end up becoming more pronounced. Some other things I could add are:
