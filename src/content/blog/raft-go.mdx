...
- Solve race condition in the election process
- Create local demo with log replication

The election race condition was occurring because 
<A 
...
Creating a 
<A 
    text="local demonstration"
    href="https://github.com/amodkala/raft/blob/master/demo.sh"
/> was also fairly straightforward. Below, you'll see that the demonstration accepts a series 
of IP addresses where the first is for the current consensus module and the following addresses 
are for its peers. I then initialize a new consensus module and channel for reading back committed 
entries before starting the consensus module in its own goroutine. It is also here where I pass in the 
cluster configuration using the 
<A
